...
export async function syncMcp(context: vscode.ExtensionContext) {
	...
	if (!currentProfilePath) {
		Logger.warn(`Current profile path not found for ${currentProfileName}`, "MCP");
		return;
	}
	...
	for (const parent of parentProfiles) {
		...
		if (mcpObj && mcpObj.mcpServers) {
			const count = Object.keys(mcpObj.mcpServers).length;
			Logger.info(`Loaded ${count} MCP servers from parent '${parent}'.`, "MCP");
	...
	try {
		...
		if (currentMcpObj && currentMcpObj.mcpServers) {
		...
		for (const [key, value] of Object.entries(aggregatedServers)) {
			if (value && typeof value === 'object') {
				...
				if (!existingKeys.has(key)) {
					...
					Logger.info(`MCP Server '${key}' exists in user config. Skipping inheritance.`, "MCP");
				}
			}
		}
		...
		Logger.info(`Synced ${Object.keys(newInheritedServers).length} inherited MCP servers.`, "MCP");
